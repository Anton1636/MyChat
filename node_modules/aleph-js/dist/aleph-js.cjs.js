...
function public_key_to_hash$1 (pub, { chain_id = 1, address_type = 1 } = {}) {
...
function address_from_hash$1 (hash, { prefix = 'NULS' } = {}) {
...
async function sign$6(prv_key, message) {
  ...
  const sigObj = secp256k1$2.sign(digest, prv_key);
...
async function new_account$5({chain_id = 1, prefix='NULS'} = {}) {
  let mnemonics =  bip39__namespace.generateMnemonic();
  return import_account$5({
    'mnemonics': mnemonics,
    'chain_id': chain_id,
    'prefix': prefix
  })
...
async function import_account$5({
  ...
  if (mnemonics) {
    let v = await bip39__namespace.mnemonicToSeed(mnemonics);
    let b = bip32__namespace.fromSeed(v);
  ...
  if (private_key !== null) {
    ...
    let hash = public_key_to_hash$1(pub, {
      'chain_id': chain_id
    });
    account['address'] = address_from_hash$1(hash, {
      'prefix': prefix
    });
    if (name)
      account['name'] = name;
...
async function new_account$3({path = "m/44'/118'/0'/0/0", prefix = "cosmos"} = {}) {
  let mnemonics = bip39__namespace.generateMnemonic();
...
async function new_account$2({format = 42} = {}) {
...
async function sign$2(account, message) {
  get_verification_buffer$2(message);
  let signed = null;
  if (account.private_key) {
...
async function sign_and_broadcast(message, account, api_server) {
  if (account) {
    ...
    if (account.type === 'NULS2') {
      await sign$6(account.private_key, message);
...
async function fetch_one(address, key, {api_server = DEFAULT_SERVER, limit = 1000} = {}) {
  let response = await axios__default["default"].get(`${api_server}/api/v0/aggregates/${address}.json?keys=${key}&limit=${limit}`);
...
async function fetch(address, {keys = null, api_server = DEFAULT_SERVER, limit = 1000} = {}) {
...
function encrypt$1(publicKeyTo, msg, opts) {
  opts = opts || {};
...
function decrypt$1(privateKey, opts) {
...
async function decrypt(account, content, { as_hex = true, as_string = true } = {}) {
  if (as_hex)
  ...
  if (curve == 'secp256k1') {
    ...
    result = await decrypt$1(eciesjs.utils.decodeHex(account['private_key']), opts);
...
async function encrypt(
  ...
  if (curve == 'secp256k1')
    ...
    result = await encrypt$1(eciesjs.utils.decodeHex(target_publickey), encrypted_content);
